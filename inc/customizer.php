...
function ezdoss_customize_register( $wp_customize ) {

    //Style Preset
    $wp_customize->add_section(
        'typography',
        array(
            'title' => __( 'Preset Styles', 'ezdoss' ),
            //'description' => __( 'This is a section for the typography', 'ezdoss' ),
            'priority' => 20,
        )
    );
    ...
    $wp_customize->add_control(
        new WP_Customize_Color_Control(
            ...
            'header_bg_color',
            array(
                'label'      => __( 'Header Banner Background Color', 'ezdoss' ),
    ...
    $wp_customize->add_control( new WP_Customize_Control($wp_customize, 'header_banner_visibility', array(
        ...
        'label'    => __('Remove Header Banner', 'ezdoss'),
        'section'    => 'header_image',
        'type'     => 'checkbox',
    ) ) );
   ...
   $wp_customize->add_section(
        ...
        array(
            'title' => __( 'Other Customizations', 'ezdoss' ),
            //'description' => __( 'This is a section for the header banner Image.', 'ezdoss' ),
            'priority' => 40,
        )
    );
    $wp_customize->add_section(
        'colors',
        array(
            'title' => __( 'Background Color', 'ezdoss' ),
            //'description' => __( 'This is a section for the header banner Image.', 'ezdoss' ),
            'priority' => 50,
    ...
    $wp_customize->add_section(
        'background_image',
        array(
            'title' => __( 'Background Image', 'ezdoss' ),
            //'description' => __( 'This is a section for the header banner Image.', 'ezdoss' ),
            'priority' => 60,
            'panel' => 'styling_option_panel',
        )
    ...
    $wp_customize->add_setting( 'cdn_assets_setting', array(
        'default' => __( 'no','ezdoss' ),
        'sanitize_callback' => 'wp_filter_nohtml_kses',
    ) );
    $wp_customize->add_control( 
        'cdn_assets',
        array(
            'label' => __( 'Use CDN for Assets', 'ezdoss' ),
            'description' => __( 'All Bootstrap Assets and FontAwesome will be loaded in CDN.', 'ezdoss' ),
    ...
    $wp_customize->add_setting( 'ezdoss_logo', array(
        //'default' => __( '', 'ezdoss' ),
        'sanitize_callback' => 'esc_url',
    ) );
    $wp_customize->add_control( new WP_Customize_Image_Control( $wp_customize, 'ezdoss_logo', array(
        'label'    => __( 'Upload Logo (replaces text)', 'ezdoss' ),
        'section'  => 'title_tagline',
        'settings' => 'ezdoss_logo',
    ) ) );

}
add_action( 'customize_register', 'ezdoss_customize_register' );

add_action( 'wp_head', 'ezdoss_customizer_css');
function ezdoss_customizer_css()
 ...
 */
function ezdoss_customize_preview_js() {
    wp_enqueue_script( 'ezdoss_customizer', get_template_directory_uri() . '/inc/assets/js/customizer.js', array( 'customize-preview' ), '20151215', true );
}
add_action( 'customize_preview_init', 'ezdoss_customize_preview_js' );
